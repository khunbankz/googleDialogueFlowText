...
//    private String responseId;
    ...
    public void speech(String inputText) {
        ...
        this.log.debug(TAG + " sessionId = " + sessionId, "Input text: " + inputText);

        boolean checkedInput = checkInput(inputText);
        if (checkedInput == true) {
            this.log.debug(TAG + " sessionId = " + sessionId, "Request text: " + requestTextInput);
            ...
            try {
                ...
                String queryText = requestMap.get(requestTextInput).getQueryText();
                String queryFullfillText = requestMap.get(requestTextInput).getFulfillmentText();
                float queryConfidenceScore = requestMap.get(requestTextInput).getIntentDetectionConfidence();
//                List<Context> queryContextList = requestMap.get(requestTextInput).getOutputContextsList();
//                this.log.debug(TAG, "queryContextList\n"+queryContextList);
//                if (!queryContextList.isEmpty()) {
//                    Context queryContext = requestMap.get(inputText).getOutputContexts(0);
//                    this.log.debug(TAG, "queryContext = " + queryContext);
//                }
//
    ...
    public void init(Config config, Log log, Object extended_arg) throws IllegalArgumentException {
        ...
        if (credentials == null) {
            ...
            try {
                ...
                this.log.debug(TAG, "credentialsGoogle : " + credentialsGoogle);
                ...
                e.printStackTrace();
                this.log.error(TAG, "unable to get credential");
                this.log.error(TAG, e);
